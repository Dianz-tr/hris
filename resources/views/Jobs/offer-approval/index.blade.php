										...
										@foreach ($managejobs as $mj)
											...
											<td>
												<h2 class="table-avatar">
													...
													<a href="profile.html">{{$mj->candidate}} {{$mj->departmen}}</a>
												</h2>
											</td>
											<td><a href="job-details.html">{{$mj->job_title}}</a></td>
											<td>{{$mj->job_type or No}}</td>
                                        @endforeach
										@foreach ($invoices as $in)
											<td> {{ number_format ($in->grand_total,2 or No)}}</td>
										@endforeach
										{{-- <td></td> --}}
										@foreach ($taxes as $tax)
											<td>{{$tax->tax_percentage or No}}%</td>
										@endforeach
											{{-- <td>{{$mj->invoices->grand_total}}</td>
											<td>{{$mj->taxes->tax_percentage}}</td> --}}
											<td>No</td>
